...
```bash
~/robot_ws/scripts/can-hub-up.sh can2 can0 can1   # 麦轮 can2 优先
```
...
- **无板载累计里程**：`/quad/wheel_states` 的 `position` 为单圈角；累计位姿由 **`quad_odom_ros2`** 对轮速积分得到

### 麦轮里程计 `quad_odom_ros2`

参考 [ros2_control `mecanum_drive_controller`](https://control.ros.org/rolling/doc/ros2_controllers/mecanum_drive_controller/doc/userdoc.html) 的做法：

1. 订阅 `/quad/wheel_states` 的 **velocity**（轮缘 rad/s）
2. 用与 `quad_teleop` **互逆**的麦轮 FK 解算车体 `vx, vy, ω`
3. 在 `odom` 坐标系积分位姿，发布 `/odom` 与 `odom → base_link` TF
...
**当前限制（仅 FL+FR 在线）：** 横移与转角为最小二乘估计，精度低于四轮全配；打滑时漂移会增大。详见 [`docs/odometry.md`](docs/odometry.md)。
...
| `quad_odom_node` | `quad_odom_ros2` | 轮速 FK + 积分 → `/odom`、`tf` |
...
| `/odom` | `nav_msgs/Odometry` | 发布 | 麦轮积分里程（`quad_odom_ros2`） |
| `tf: odom → base_link` | — | 广播 | 与 `/odom` 同步 |
...
| [`config/quad_odom_params.yaml`](config/quad_odom_params.yaml) | 里程计 FK、TF、协方差 |
...
| 现象 | 排查 |
|------|------|
| 重启后页面已连接但摇杆无效 | **只重启了机器人节点、Web 未重启** → `~/quad_remote/restart-web.sh` 或 `start-all.sh`；手机强制刷新 |
| 手机显示 ROS 未连接 | `ros2 node list` 确认三节点；`curl localhost:8765/health` |
| 摇杆无反应 | 是否横屏；`can_control` 是否为 true（只开一个标签页）；检查 `/cmd_vel` 订阅 |
...
| 日期 | 内容 |
|------|------|
| 2026-09-17 | 手机遥控 `quad_remote`、50Hz 低延迟链路、README/脚本文档完善 |
| 2026-09-15 | 四足全机脚本整合、`run-quad-all.sh`、C620 双轮在线 |
