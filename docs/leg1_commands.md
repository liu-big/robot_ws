...
| 关节 | 电机 | CAN | 节点 | 说明 |
|------|------|-----|------|------|
| 抬升 `leg1_lift` | 灵足 RS03 (ID `1`) | **can0** | `quad_rs_ros2` | 位置/运控保持 |
| 趴下/起身 `leg1_crouch` | 灵足 RS03 (ID `5`) | **can1** | `quad_rs_ros2` | 同上 |
| 麦轮 `c620_motor` | 大疆 C620 + M3508 (ID `1`) | **can2** | `c620_ros2` | 速度闭环 + 零速锁位 |

> 推荐 `quad_rs_ros2`；`leg1_dual_ros2` 与之二选一。不要同时跑两个灵足节点。
