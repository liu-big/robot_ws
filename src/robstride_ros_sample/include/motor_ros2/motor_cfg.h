...
public:
  ...
  bool receive_status_frame(double timeout_sec = 0.05);
  ...
  // 失能→切运控→使能→发保持帧；返回是否收到反馈
  bool init_motion_mode(float hold_kp = 5.0f, float hold_kd = 0.5f);
  // 切 CSP 位置模式并设目标角（适合发角度）
  bool init_csp_mode(float speed_rad_s, float angle_rad);
  std::tuple<float, float, float, float>
  send_csp_hold(float speed_rad_s, float angle_rad);
  void drain_rx();
  uint8_t get_motor_mode() const { return motor_mode_; }
  bool is_running() const { return motor_mode_ == 2; }
  const char *motor_mode_name() const;
  void set_can_debug(bool on) { can_debug_ = on; }
  // 仅发运控保持帧（不再反复 disable/切模式）
  std::tuple<float, float, float, float>
  send_motion_hold(float position_rad, float velocity_rad_s = 0.0f,
                   float kp = 0.5f, float kd = 0.1f);
...
public:
  ...
  uint8_t error_code;
  uint8_t pattern;
  uint8_t motor_mode_{0}; // can_id bit22-23: 0=Reset 1=Cali 2=Run
  bool can_debug_{false};
  bool csp_mode_ready_{false};
