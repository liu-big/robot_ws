...
void RobStrideMotor::drain_rx() {
  struct timeval tv{};
  tv.tv_sec = 0;
  tv.tv_usec = 2000;
  setsockopt(socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
  struct can_frame frame{};
  while (recv(socket_fd, &frame, sizeof(struct can_frame), 0) > 0) {
  }
}

const char *RobStrideMotor::motor_mode_name() const {
  switch (motor_mode_) {
  case 0:
    return "Reset";
  case 1:
...
bool RobStrideMotor::init_csp_mode(float speed_rad_s, float angle_rad) {
  drain_rx();
  Disenable_Motor(0);
  usleep(5000);
  send_csp_hold(speed_rad_s, angle_rad);
  return true;
}

std::tuple<float, float, float, float>
RobStrideMotor::send_csp_hold(float speed_rad_s, float angle_rad) {
  return RobStrite_Motor_PosCSP_control(speed_rad_s, angle_rad);
}

bool RobStrideMotor::init_motion_mode(float hold_kp, float hold_kd) {
  drain_rx();
  Disenable_Motor(1); // 清故障
  usleep(5000);
  Set_RobStrite_Motor_parameter(0X7005, static_cast<float>(move_control_mode),
                                Set_mode);
  usleep(3000);
  enable_motor();
  usleep(5000);
  drw.run_mode.data = move_control_mode;

  // 使能后持续发保持帧，直到收到有效反馈
  bool got_feedback = false;
  for (int i = 0; i < 30; ++i) {
    send_motion_hold(position_, 0.0f, hold_kp, hold_kd);
    got_feedback = true;
    if (is_running()) {
      return true;
    }
...
std::tuple<float, float, float, float>
RobStrideMotor::send_motion_hold(float position_rad, float velocity_rad_s,
                                 float kp, float kd) {
  struct can_frame frame{};
  frame.can_id =
      (Communication_Type_MotionControl << 24) |
  ...
  const auto &lim =
      ACTUATOR_OPERATION_MAPPING.at(static_cast<ActuatorType>(actuator_type));
  uint16_t pos = float_to_uint(position_rad, -lim.position, lim.position, 16);
  uint16_t vel = float_to_uint(velocity_rad_s, -lim.velocity, lim.velocity, 16);
  uint16_t kp_u = float_to_uint(kp, 0.0f, lim.kp, 16);
  uint16_t kd_u = float_to_uint(kd, 0.0f, lim.kd, 16);
  ...
  if (write(socket_fd, &frame, sizeof(frame)) != sizeof(frame)) {
    perror("send_motion_hold failed");
  }
  receive_status_frame();
  return std::make_tuple(position_, velocity_, torque_, temperature_);
}
...
RobStrideMotor::RobStrite_Motor_PosCSP_control(float Speed, float Angle) {
  ...
  Motor_Set_All.set_angle = Angle;
  if (can_debug_) {
    std::cout << "csp speed: " << Motor_Set_All.set_speed
              << " angle: " << Motor_Set_All.set_angle << std::endl;
  }
  ...
  if (!csp_mode_ready_) {
    ...
    enable_motor();
    usleep(5000);
    drw.run_mode.data = PosCSP_control_mode;
    Motor_Set_All.set_motor_mode = PosCSP_control_mode;
    csp_mode_ready_ = true;
  }
