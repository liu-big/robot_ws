...
(() => {
  ...
  function paintTelemetry() {
    ...
    const odomOk = fresh && state?.odom_ok;
    const yawDeg = odomOk ? String(Math.round(state.yaw * 180 / Math.PI)) : '—';
    for (const [id, key, dec] of [['odomX', 'x', 2], ['odomY', 'y', 2], ['odomDist', 'dist', 2]]) {
      text($(id), odomOk ? format(state[key], dec) : '—');
    }
    text($('odomYaw'), yawDeg);
    text($('detailOdomX'), odomOk ? format(state.x, 3) : '—');
    text($('detailOdomY'), odomOk ? format(state.y, 3) : '—');
    text($('detailOdomDist'), odomOk ? format(state.dist, 3) : '—');
    text($('detailOdomYaw'), yawDeg);
    $('odomReset').disabled = !ui.connected || !ui.canControl;
    ...
    ws.addEventListener('message', event => {
      ...
      if (message.type === 'hello') {
        ...
        if (!ui.state) {
          ...
          ui.state.odom_ok = message.odom_ok;
          if (message.x !== undefined) {
            ui.state.x = message.x;
            ui.state.y = message.y;
            ui.state.yaw = message.yaw;
            ui.state.dist = message.dist;
          }
          if (message.joints) {
  ...
  $('odomReset').addEventListener('click', () => {
    ...
    if (sendNow({ type: 'odom_reset' })) toast('里程计归零中…');
  });
